...
searchButton.addEventListener("click", function () {
  ...
  let matchedCard = null; // To store the matched card
  ...
  cardElements.forEach(function (card) {
    ...
    if (title.includes(searchValue)) {
      ...
      matchedCard = card; // Set the matched card
    } else {
  ...
  // Scroll to the matched card if one exists
  if (matchedCard) matchedCard.scrollIntoView({ behavior: "smooth", block: "start" });
});
});
